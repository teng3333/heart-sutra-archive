/* HSE/AN 生きた背景アート — OGSホーム用モジュール(prototype v1.4より生成)
   球体生態系・4顔巡回・進化する遺伝子・色テーマ・天候・天体・隠れキャラ。
   使い方: <script src="src/js/living-bg.js" defer></script>
   ・canvasは自前生成し body 先頭へ固定配置(z-index:0 / pointer-events:none)
   ・prefers-reduced-motion では静止1フレームのみ描画 */
(function(){
...
Object.assign(cv.style, { position:'fixed', inset:'0', width:'100%', height:'100%',
  ...
  opacity:'0.62' });   // 背景として一歩引く(本文可読性を優先)
...
function drawCelestial(t, vis, glow, tintArr){
  ...
  const a0 = (0.10 + vis*0.16) * (0.85 + Math.sin(t*0.12 + c.seed)*0.15);  // 呼吸
  if (a0 < 0.012) return;
...
function draw(now){
  ...
  if (auto && pi === 6 && lastPi !== 6){ evolve(); nextTheme(); pickCelestial(); rebuild(); }
  lastPi = pi;
...
// 調整・検証用ハンドル(本番でも無害。ライフサイクル位置や成長を外から確認できる)
window.__livingBG = {
  ...
  setCelestial(k){ celestial.kind = k; },
  rebuild(){ rebuild(); },
};
