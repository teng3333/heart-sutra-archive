...
function pickWeather(t){
  const r = Math.random();
  weather = r<0.16 ? 'none'    : r<0.26 ? 'rain'    : r<0.36 ? 'sakura'
          : r<0.46 ? 'snow'    : r<0.56 ? 'leaves'  : r<0.64 ? 'digital'
          : r<0.72 ? 'bubbles' : r<0.80 ? 'stars'   : r<0.87 ? 'fish'
          : r<0.94 ? 'butterfly' : 'ripple';
  ...
  WP.forEach(p => {
    ...
    if (weather === 'fish'){                                       // 左右に泳ぐ小魚
      ...
      p.wigS = 5 + Math.random()*5;                                // 尾を振る速さ
      p.bob = Math.random()*9;                                     // 上下のゆらぎ
    ...
    if (weather === 'butterfly'){                                  // ひらひら舞う蝶
      ...
      p.wx = Math.random()*9; p.wy = Math.random()*9;              // 遊泳の位相
      p.wxs = 0.20 + Math.random()*0.35;                           // ふらつきの速さ
    ...
    if (weather === 'ripple'){                                     // 水滴と広がる波紋
      ...
      p.rings = 2 + (Math.random()*3|0);                           // 重なる輪の数
    }
  });
}
function drawWeather(t, tint){
  ...
  // ── 小魚:球のまわりを左右に泳ぐ。時折向きを変え、尾を振る ──
  if (weather === 'fish'){
    for (const p of WP){
      p.wig += 0.016 * p.wigS;
      p.bob += 0.012;
      p.turn -= 0.016;
      if (p.turn <= 0){ p.dir *= -1; p.turn = 5 + Math.random()*12; }   // 気まぐれに反転
      ...
      const wig = Math.sin(p.wig);
      ctx.save(); ctx.translate(px, py); ctx.scale(p.dir, 1);
      // 胴(銀に淡く発光)
      const g = ctx.createLinearGradient(-s, -s*0.4, s, s*0.4);
      g.addColorStop(0, `rgba(150,175,200,${a*0.55})`);
      g.addColorStop(0.5, `rgba(${tint},${a})`);
      g.addColorStop(1, `rgba(120,150,180,${a*0.6})`);
      ctx.fillStyle = g;
      ctx.beginPath(); ctx.ellipse(0, 0, s, s*0.38, wig*0.10, 0, 7); ctx.fill();
      // 尾びれ(振れる)
      ctx.fillStyle = `rgba(${tint},${a*0.75})`;
      ctx.beginPath();
      ctx.moveTo(-s*0.85, 0);
      ctx.lineTo(-s*1.75, -s*0.42 + wig*s*0.30);
      ctx.lineTo(-s*1.75,  s*0.42 + wig*s*0.30);
      ctx.closePath(); ctx.fill();
      // 背びれと目
      ctx.beginPath();
      ctx.moveTo(-s*0.1, -s*0.34); ctx.lineTo(s*0.25, -s*0.72); ctx.lineTo(s*0.42, -s*0.28);
      ctx.closePath(); ctx.fill();
      ctx.fillStyle = `rgba(255,255,255,${a*0.9})`;
      ctx.beginPath(); ctx.arc(s*0.55, -s*0.06, s*0.11, 0, 7); ctx.fill();
      ctx.restore();
    }
    return;
  }

  // ── 蝶:色とりどりに、ひらひらと不規則に舞う ──
  if (weather === 'butterfly'){
    for (const p of WP){
      p.flap += 0.016 * p.flapS;
      p.wx += 0.016 * p.wxs; p.wy += 0.016 * p.wys;
      // 二重の正弦で、直線的でない気まぐれな軌跡を作る
      p.nx += (Math.sin(p.wx) * 0.0016 + Math.sin(p.wx*2.3) * 0.0009);
      ...
      const open = Math.abs(Math.sin(p.flap));       // 羽ばたき(0=閉じ 1=開き)
      const tilt = Math.sin(p.wx) * 0.5;
      ctx.save(); ctx.translate(px, py); ctx.rotate(tilt);
      for (const side of [-1, 1]){
        ctx.save(); ctx.scale(side * (0.25 + open*0.75), 1);       // 開閉で幅が変わる
        ctx.fillStyle = `hsla(${p.hue},85%,72%,${a})`;
        ctx.beginPath(); ctx.ellipse(s*0.62, -s*0.34, s*0.68, s*0.46, -0.35, 0, 7); ctx.fill();
        ctx.fillStyle = `hsla(${(p.hue+28)%360},82%,62%,${a*0.92})`;
        ctx.beginPath(); ctx.ellipse(s*0.50,  s*0.32, s*0.50, s*0.36,  0.42, 0, 7); ctx.fill();
        ctx.restore();
      }
      ctx.strokeStyle = `rgba(24,20,26,${a*0.85})`; ctx.lineWidth = 0.9;  // 胴
  ...
  // ── 水滴と波紋:落ちて着水し、幾重もの輪が重なりながら広がる ──
  if (weather === 'ripple'){
    for (const p of WP){
      if (p.delay > 0){ p.delay -= 0.016; continue; }
      if (p.state === 0){                                   // 落下中
        p.dropY += 0.016 * (0.9 + p.spd*0.6);
        if (p.dropY >= p.tgt){ p.state = 1; p.rt = 0; }
          ...
          ctx.beginPath();                                   // 雫(下がふくらむ)
          ctx.ellipse(px, py, 1.1 + p.s, 2.0 + p.s*1.6, 0, 0, 7); ctx.fill();
        }
      } else {                                              // 着水後:広がる波紋
        p.rt += 0.016;
        const ny = p.tgt;
        ...
        for (let k = 0; k < p.rings; k++){
          ...
          const life = age / 2.6;
          if (life >= 1) continue;
          const rr = (9 + p.s*16) * (0.15 + life*2.9) * (1 + k*0.1);
          const a = intens * Math.pow(1 - life, 1.25) * 0.95 * (0.55 + p.s*0.45);
          if (a < 0.02) continue;
          ctx.strokeStyle = `rgba(${tint},${a})`;
          ctx.lineWidth = Math.max(0.7, 2.4 * (1 - life));
          ctx.beginPath();
          ctx.ellipse(px, py, rr, rr*0.34, 0, 0, 7);         // 水面なので扁平に
          ctx.stroke();
        }
        // 着水の閃光
        if (p.rt < 0.3){
          const fa = intens * (1 - p.rt/0.3) * 0.85;
          const g2 = ctx.createRadialGradient(px, py, 0, px, py, 11);
          g2.addColorStop(0, `rgba(235,245,255,${fa})`);
          g2.addColorStop(1, 'rgba(0,0,0,0)');
          ctx.fillStyle = g2;
          ctx.beginPath(); ctx.arc(px, py, 11, 0, 7); ctx.fill();
        }
        if (p.rt > 2.6 + p.rings*0.42){                      // 次の一滴へ
          p.nx = (Math.random()*2-1)*1.05;
          p.tgt = (Math.random()*2-1)*0.95;
          p.dropY = p.tgt - 0.5 - Math.random()*1.1;
          p.state = 0; p.delay = Math.random()*4; p.rings = 2 + (Math.random()*3|0);
        }
      }
    }
    return;
