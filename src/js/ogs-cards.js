...
(function () {
  ...
  /* 一覧の取得を待つ上限。8秒では足りなかった(2026-09-21 高尾さん「新着とAN選が消えてる」)。
     読めないとAN選も新着も枠ごと隠れるので、遅いと丸ごと消えたように見える。

     本番の実測(2026-09-21):
       /api/archive  最初の1バイトまで 15.3秒、転送は0.05秒。12.7〜25.6秒の幅
       同じ問い合わせをDBへ直接  0.10〜0.39秒(108行)
     つまり遅いのは通信でもDBでもなく、Railway側のアプリの中。
     直すべきはそちらだが、直るまで家の玄関が欠けたままなのは困る。
     ここは30秒まで待って、枠が出ることを優先する。 */
